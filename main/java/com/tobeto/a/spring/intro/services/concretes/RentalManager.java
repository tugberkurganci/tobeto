...
public class RentalManager implements RentalService {
    ...
    public int createRental(List<CreateRentalRequest> rentalRequests) {
       ...
       rentalRequests.forEach(rentalRequest -> {
           ...
           rental.setUser(userService.getOriginalUserById(rentalRequest.getUserId()));
               ...
               try {
                   ...
                   id.set(savedRental.getId());
               } catch (Exception ex) {
                   // Optimistic locking hatası alındığında işleme devam edebilirsiniz.
                   throw new IllegalStateException("Rental kaydı yapılamadı. Çakışma olabilir.");
               }
         ;}});
    ...
    @Override
    public double calculateReservationPrice(CreateRentalRequest rentalRequest) {
        Car car=carService.getOrginalCarById(rentalRequest.getCarId());

        long rentalDays = ChronoUnit.DAYS.between(rentalRequest.getRentalDate(),rentalRequest.getReturnDate());


        double totalCost = rentalDays * car.getDailyPrice();

        if(rentalDays==0)return car.getDailyPrice();
        return totalCost;
